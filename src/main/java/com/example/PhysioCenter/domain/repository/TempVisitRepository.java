...
public interface TempVisitRepository extends CrudRepository<TempVisit, Long> {
    ...
    @Query(value = "SELECT * FROM fizjoterapia.temp_termin AS t WHERE t.id_fizjoterapeuty = :physioId AND t.data = :date AND t.czas_od = :timeFrom AND t.czas_do = :timeTo", nativeQuery = true)
    ...
    @Query(value = "INSERT INTO fizjoterapia.temp_termin (id_fizjoterapeuty, id_pacjenta, wolny, data, czas_od, czas_do) VALUES (:physioId, :patientId, false, :date, :timeFrom, :timeTo);", nativeQuery = true)
